...
int main(){
    ...
    if(pick == 1){
        for(int i = 0; i < sizeV; ++i){
            ...
            countingSort(unsorted[i], MaxV);
            auto stop = std::chrono::high_resolution_clock::now();
            auto duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
            double total_time = duration.count() / 1'000'000.0;

            std::cout << "Runtime for " << sizes[i] << " Unsorted vector size: " << total_time << " with countingSort" << "\n";
            std::cout << "SORTED?: " << std::is_sorted(unsorted[i].begin(),unsorted[i].end()) << " " << "\n";
        }
    }else if(pick == 2){
        for(int i = 0; i < sizeV; ++i){
            auto start = std::chrono::high_resolution_clock::now();
            countingSort(ascending[i], MaxV);
            auto stop = std::chrono::high_resolution_clock::now();
            auto duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
            double total_time = duration.count() / 1'000'000.0;

            std::cout << "Runtime for " << sizes[i] << " Ascending vector size: " << total_time << " with countingSort" << "\n";
            std::cout << "SORTED?: " << std::is_sorted(ascending[i].begin(),ascending[i].end()) << " " << "\n";
        }
    }else if(pick == 3){
        for(int i = 0; i < sizeV; ++i){
            auto start = std::chrono::high_resolution_clock::now();
            countingSort(descending[i], MaxV);
            auto stop = std::chrono::high_resolution_clock::now();
            auto duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
            double total_time = duration.count() / 1'000'000.0;

            std::cout << "Runtime for " << sizes[i] << " Descending vector size: " << total_time << " with countingSort" << "\n";
