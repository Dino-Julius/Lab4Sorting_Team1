...
int main(){
    ...
    if(pick == 1){
        for(int i = 0; i < sizeV; ++i){
            ...
            selectionSort(unsorted[i]);
            auto stop = std::chrono::high_resolution_clock::now();
            auto duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
            double total_time = duration.count() / 1'000'000.0;

            std::cout << "Runtime for " << sizes[i] << " Unsorted vector size: " << total_time << " with selectionSort" << "\n";
            std::cout << "SORTED?: " << std::is_sorted(unsorted[i].begin(),unsorted[i].end()) << " " << "\n";
        }
    }else if(pick == 2){
        for(int i = 0; i < sizeV; ++i){
            auto start = std::chrono::high_resolution_clock::now();
            selectionSort(ascending[i]);
            auto stop = std::chrono::high_resolution_clock::now();
            auto duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
            double total_time = duration.count() / 1'000'000.0;

            std::cout << "Runtime for " << sizes[i] << " Ascending vector size: " << total_time << " with selectionSort" << "\n";
            std::cout << "SORTED?: " << std::is_sorted(ascending[i].begin(),ascending[i].end()) << " " << "\n";
        }
    }else if(pick == 3){
        for(int i = 0; i < sizeV; ++i){
            auto start = std::chrono::high_resolution_clock::now();
            selectionSort(descending[i]);
            auto stop = std::chrono::high_resolution_clock::now();
            auto duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
            double total_time = duration.count() / 1'000'000.0;

            std::cout << "Runtime for " << sizes[i] << " Descending vector size: " << total_time << " with selectionSort" << "\n";
