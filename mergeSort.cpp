...
int main(){
    ...
    if(pick == 1){
        for(int i = 0; i < sizeV; ++i){
            scratch.resize(sizes[i]);
            auto start = std::chrono::high_resolution_clock::now();
            mergeSort(unsorted[i], scratch,0, unsorted[i].size() - 1);
            auto stop = std::chrono::high_resolution_clock::now();
            auto duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
            double total_time = duration.count() / 1'000'000.0;

            std::cout << "Runtime for " << sizes[i] << " Unsorted vector size: " << total_time << " with mergeSort" << "\n";
        ...
        for(int i = 0; i < sizeV; ++i){
            scratch.resize(sizes[i]);
            auto start = std::chrono::high_resolution_clock::now();
            mergeSort(ascending[i],  scratch,0, unsorted[i].size() - 1);
            auto stop = std::chrono::high_resolution_clock::now();
            auto duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
            double total_time = duration.count() / 1'000'000.0;

            std::cout << "Runtime for " << sizes[i] << " Ascending vector size: " << total_time << " with mergeSort" << "\n";
        ...
        for(int i = 0; i < sizeV; ++i){
            scratch.resize(sizes[i]);
            auto start = std::chrono::high_resolution_clock::now();
            mergeSort(descending[i], scratch,0, unsorted[i].size() - 1);
            auto stop = std::chrono::high_resolution_clock::now();
            auto duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
            double total_time = duration.count() / 1'000'000.0;

            std::cout << "Runtime for " << sizes[i] << " Descending vector size: " << total_time << " with mergeSort" << "\n";
